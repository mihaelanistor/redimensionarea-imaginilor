...
switch metodaSelectareDrum
    ...
    case 'greedy'
        ...
        linia = 1;
        coloana = find(E(linia,:) == min(E(linia,:)),1);  
        d(1,:) = [linia coloana];
        ...
        for i = 2:size(d,1)
            ...
            %coloana depinde de coloana pixelului anterior
            if d(i-1,2) == 1 %pixelul este localizat la marginea din stanga
                v = [E(linia,d(i-1,2)) E(linia,d(i-1,2)+1)];
                coloana = find(v == min(v),1);
                               
            elseif d(i-1,2) == size(E,2) %pixelul este la marginea din dreapta
                v = [E(linia,d(i-1,2)-1) E(linia,d(i-1,2))];
                pozMinim = find(v == min(v),1);
                coloana = coloana + pozMinim - 2;
                                
            else
                v = [E(linia,d(i-1,2)-1) E(linia,d(i-1,2)) E(linia,d(i-1,2)+1)];
                pozMinim = find(v == min(v),1);
                coloana = coloana + pozMinim - 2;
